({
    ...
    acceptCurrentBid : function(component, event, helper) {
        ...
        if(dauc.RecordType.Name === 'Generic Auction') {
            if(dauc.Current_Bid_Price__c == null || dauc.Current_Bid_Price__c == undefined || dauc.Current_Bid_Price__c <= 0) {
                alert("Please give Your Bid to Submit.");
                return;
            }

            if(dauc.Current_Bid_Price__c != null && dauc.Current_Bid_Price__c != undefined && dauc.Current_Bid_Price__c > dauc.Base_Price__c) {
                alert("You can not give your bid more than Base Price.");
                return;
            }
        }
